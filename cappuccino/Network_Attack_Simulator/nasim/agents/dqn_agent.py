...
class DQNAgent:
    ...
    def train(self):
        ...
        while self.steps_done < self.training_steps:
            ...
            f.write(f"\tsteps done = {self.steps_done} / "
                    f"{self.training_steps}")
            print(f"\treturn = {ep_return}")
            f.write(f"\treturn = {ep_return}")
            print(f"\tgoal = {goal}")
            f.write(f"\tgoal = {goal}")
            print(f"\tepisode_time = {end_time-start_time}")
            f.write(f"\tepisode_time = {end_time-start_time}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--training_steps", type=int, default=40000,
                        help="training steps (default=40000)")
